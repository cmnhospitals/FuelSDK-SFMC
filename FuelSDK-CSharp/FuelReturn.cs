...
namespace FuelSDK
{
	...
	public abstract class FuelReturn
	{
		...
		protected TResult[] ExecuteAPI<TResult, TObject>(Func<APIObject, TObject> select, Func<ETClient, TObject[], ExecuteAPIResponse<TResult>> func, params APIObject[] objs)
		{
			...
			using (new OperationContextScope(client.SoapClient.InnerChannel))
			{
				...
				//var retrieveRequest1 = new RetrieveRequest1(new RetrieveRequest
				//{
				//	ObjectType = "BusinessUnit",
				//	Properties = new[] { "ID", "Name" }
				//});

				//var retrieveResponse = client.SoapClient.RetrieveAsync(retrieveRequest1);
