	...
	public class PatchReturn : FuelReturn
	{
		...
		public PatchReturn(APIObject objs)
		{
			...
			var response = ExecuteAPI((client, o) =>
			{
				...
				var updateRequest = new UpdateRequest(options, o);
				var updateResponse = client.SoapClient.UpdateAsync(updateRequest).Result;

				return new ExecuteAPIResponse<UpdateResult>(updateResponse.Results, updateResponse.RequestID, updateResponse.OverallStatus);
			}, objs);
				...
				if (response.GetType() == typeof(UpdateResult[]) && response.Length > 0)
				{
					Results = response.Cast<UpdateResult>().Select(x => new ResultDetail
					{
						...
						Object = (x.Object != null
							? (objs.GetType().ToString().Contains("ET_") ? TranslateObject2(x.Object) : TranslateObject(x.Object))
							: null),
